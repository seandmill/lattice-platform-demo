...
export const highLevelNodes: LatticeNode[] = [
  ...
  { id: 'tool_registry', position: { x: 20, y: 115 }, parentId: 'ctrl_plane', extent: 'parent', data: { label: 'Tool Registry', componentType: 'control', status: 'idle', canDrillDown: false, payload: samplePayloads.toolRegistryEntry } },
  { id: 'workflow_registry', position: { x: 230, y: 115 }, parentId: 'ctrl_plane', extent: 'parent', data: { label: 'Workflow Registry', componentType: 'control', status: 'idle', canDrillDown: false, payload: samplePayloads.workflowRegistryEntry } },
  ...
  { id: 'tool_gateway', position: { x: 280, y: 210 }, parentId: 'runtime_plane', extent: 'parent', data: { label: 'Tool Gateway', description: 'MCP Client', componentType: 'runtime', status: 'idle', canDrillDown: true, drillDownTarget: 'tool_gateway_component' } },
  ...
  // MCP Servers (Domain Capabilities)
  { id: 'mcp_plane', type: 'group', position: { x: 870, y: 520 }, style: { width: 450, height: 120, backgroundColor: 'rgba(139, 92, 246, 0.1)', borderRadius: 8 }, data: { label: 'Domain MCP Servers', componentType: 'external', status: 'idle', canDrillDown: false } },
  { id: 'mcp_servicing_hl', position: { x: 20, y: 50 }, parentId: 'mcp_plane', extent: 'parent', data: { label: 'Cases MCP', componentType: 'external', status: 'idle', canDrillDown: false, description: 'Lookup, Status' } },
  { id: 'mcp_documents_hl', position: { x: 170, y: 50 }, parentId: 'mcp_plane', extent: 'parent', data: { label: 'Documents MCP', componentType: 'external', status: 'idle', canDrillDown: false, description: 'Search, Fetch' } },
  { id: 'mcp_risk_hl', position: { x: 310, y: 50 }, parentId: 'mcp_plane', extent: 'parent', data: { label: 'Analytics MCP', componentType: 'external', status: 'idle', canDrillDown: false, description: 'Scoring, Calc' } },
]
...
export const highLevelEdges: LatticeEdge[] = [
  ...
  // MCP connections
  { id: 'tool_gateway-mcp_servicing_hl', source: 'tool_gateway', target: 'mcp_servicing_hl', data: { status: 'idle', label: 'MCP' } },
  { id: 'tool_gateway-mcp_documents_hl', source: 'tool_gateway', target: 'mcp_documents_hl', data: { status: 'idle', label: 'MCP' } },
  { id: 'tool_gateway-mcp_risk_hl', source: 'tool_gateway', target: 'mcp_risk_hl', data: { status: 'idle', label: 'MCP' } },
...
export const highLevelSequence: SequenceStep[] = [
  ...
  { id: 'step7', from: 'workflow_registry', to: 'ai_gateway', label: 'workflow definition', isReturn: true, payload: samplePayloads.workflowRegistryEntry },
  ...
  { id: 'step14', from: 'orchestration_engine', to: 'tool_gateway', label: 'Request tool call' },
  { id: 'step15', from: 'tool_gateway', to: 'mcp_servicing_hl', label: 'MCP tools/call', payload: samplePayloads.mcpToolCall },
  { id: 'step16', from: 'mcp_servicing_hl', to: 'tool_gateway', label: 'MCP response', isReturn: true, payload: samplePayloads.mcpToolResponse },
  { id: 'step17', from: 'tool_gateway', to: 'orchestration_engine', label: 'Tool result', isReturn: true },
  { id: 'step18', from: 'orchestration_engine', to: 'model_gateway', label: 'Generate response' },
  { id: 'step19', from: 'model_gateway', to: 'approved_models', label: 'Inference request' },
  { id: 'step20', from: 'approved_models', to: 'model_gateway', label: 'Model output', isReturn: true },
  { id: 'step21', from: 'model_gateway', to: 'orchestration_engine', label: 'Structured response', isReturn: true, payload: samplePayloads.modelResponse },
  { id: 'step22', from: 'orchestration_engine', to: 'ai_gateway', label: 'Result', isReturn: true },
  { id: 'step23', from: 'ai_gateway', to: 'audit_log', label: 'Write audit events', isAsync: true },
  { id: 'step24', from: 'ai_gateway', to: 'observability', label: 'Emit metrics', isAsync: true },
  { id: 'step25', from: 'ai_gateway', to: 'review_workbench', label: '200 OK', isReturn: true, payload: samplePayloads.finalResponse },
]
